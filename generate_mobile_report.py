    ...
    <script>
        ...
        function renderWaterfallCharts(compId) {{
            ...
            ['Q1', 'Q2', 'Q3'].forEach((qName, idx) => {{
                ...
                const text_values = [];
                const keys = Object.keys(waterfall_data);
                for (let i = 0; i < keys.length; i++) {{
                    const key = keys[i];
                    const val = Object.values(waterfall_data)[i];
                    ...
                    if (key === 'Doanh Thu') {{
                        percentage = '100%';
                    }} else if (key === 'Lãi Gộp' || key === 'LN Trước Thuế') {{
                        // For totals, show as % of revenue
                        percentage = ((absVal / revenue) * 100).toFixed(1) + '%';
                    }} else {{
                        // For expenses, show as % of revenue
                        percentage = ((absVal / revenue) * 100).toFixed(1) + '%';
                    }}
                    
                    text_values.push(formatted + ' ' + percentage);
                }}
