...
<body>
    ...
    <script>
        ...
        function renderExpenseStructureQuarterlyChart(compId) {{
            ...
            // Tạo traces cho từng loại chi phí (2024 và 2025)
            const traces = [
                {{
                    name: 'CP Bán hàng 2024',
                    x: quarters,
                    y: quarters.map(q => exp2024.selling[q]),
                    type: 'bar',
                    marker: {{ color: '#E03A3E', opacity: 0.6 }},
                    legendgroup: 'selling'
                }},
                {{
                    name: 'CP Bán hàng 2025',
                    x: quarters,
                    y: quarters.map(q => exp2025.selling[q]),
                    type: 'bar',
                    marker: {{ color: '#E03A3E', opacity: 0.9 }},
                    legendgroup: 'selling'
                }},
                {{
                    name: 'CP Quản lý 2024',
                    x: quarters,
                    y: quarters.map(q => exp2024.admin[q]),
                    type: 'bar',
                    marker: {{ color: '#F5A623', opacity: 0.6 }},
                    legendgroup: 'admin'
                }},
                {{
                    name: 'CP Quản lý 2025',
                    x: quarters,
                    y: quarters.map(q => exp2025.admin[q]),
                    type: 'bar',
                    marker: {{ color: '#F5A623', opacity: 0.9 }},
                    legendgroup: 'admin'
                }},
                {{
                    name: 'CP Khác 2024',
                    x: quarters,
                    y: quarters.map(q => exp2024.other[q]),
                    type: 'bar',
                    marker: {{ color: '#9B9B9B', opacity: 0.6 }},
                    legendgroup: 'other'
                }},
                {{
                    name: 'CP Khác 2025',
                    x: quarters,
                    y: quarters.map(q => exp2025.other[q]),
                    type: 'bar',
                    marker: {{ color: '#9B9B9B', opacity: 0.9 }},
                    legendgroup: 'other'
                }}
            ];
